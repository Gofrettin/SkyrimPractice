...
#pragma region gameoverlay.dll Hook
	...
	presentAddr = ((uintptr_t)find_signature("FF 15 ? ? ? ? 8B F0 85 FF", (uint8_t*)gameOverlayInfo.lpBaseOfDll, gameOverlayInfo.SizeOfImage)) + 2;
	resetAddr = ((uintptr_t)find_signature("C7 45 ? ? ? ? ? FF 15 ? ? ? ? 8B D8", (uint8_t*)gameOverlayInfo.lpBaseOfDll, gameOverlayInfo.SizeOfImage)) + 9;
